...
function extractMarkdown(html: string, baseUrl: string): string {
  ...
  md = md.replace(/<a[^>]*href\s*=\s*["']([^"']*)["'][^>]*>([\s\S]*?)<\/a>/gi, (_m, href, text) => {
    const resolved = href.startsWith("http") ? href : `https://${domain}${href.startsWith("/") ? "" : "/"}${href}`;
    return `[${text}](${resolved})`;
  });

  md = md.replace(/<img[^>]*src\s*=\s*["']([^"']*)["'][^>]*alt\s*=\s*["']([^"']*)["'][^>]*\/?>/gi, (_m, src, alt) => {
    const resolved = src.startsWith("http") ? src : `https://${domain}${src.startsWith("/") ? "" : "/"}${src}`;
    return `![${alt}](${resolved})`;
  });
  md = md.replace(/<img[^>]*alt\s*=\s*["']([^"']*)["'][^>]*src\s*=\s*["']([^"']*)["'][^>]*\/?>/gi, (_m, alt, src) => {
    const resolved = src.startsWith("http") ? src : `https://${domain}${src.startsWith("/") ? "" : "/"}${src}`;
    return `![${alt}](${resolved})`;
  });
...
function contentBySelector(html: string, selector: string): string {
  ...
  const matches = [...html.matchAll(regex)].map((m) => m[1].replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim());
  return matches.join("\n---\n");
}
...
const scrapeUrlTool: Tool = {
  definition: {
    ...
    params: [
      { name: "url", type: "string", description: "The URL to scrape (must be HTTP or HTTPS)", required: true },
      { name: "format", type: "string", description: "Output format for the scraped content", required: false, enum: ["text", "markdown", "html"], defaultValue: "text" },
      { name: "selector", type: "string", description: "CSS selector to extract specific content from the page", required: false },
      { name: "include_metadata", type: "boolean", description: "Whether to include page metadata", required: false, defaultValue: true },
    ],
...
const scrapeCrawlTool: Tool = {
  definition: {
    ...
    params: [
      { name: "start_url", type: "string", description: "The starting URL for the crawl", required: true },
      { name: "max_pages", type: "number", description: "Maximum number of pages to crawl (default: 10)", required: false, defaultValue: 10 },
      { name: "max_depth", type: "number", description: "Maximum crawl depth from the start URL (default: 2)", required: false, defaultValue: 2 },
      { name: "same_domain", type: "boolean", description: "Only follow links on the same domain (default: true)", required: false, defaultValue: true },
      { name: "selector", type: "string", description: "CSS selector to extract specific content from each crawled page", required: false },
    ],
    capabilities: ["network:fetch"],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      const maxPages = typeof rawMaxPages === "number" && rawMaxPages > 0 ? rawMaxPages : (config.defaultMaxPages || 10);
...
const scrapeSearchTool: Tool = {
  definition: {
    ...
    params: [
      ...
      { name: "max_results", type: "number", description: "Maximum number of results to return (default: 10)", required: false, defaultValue: 10 },
      { name: "engine", type: "string", description: "Search engine to use", required: false, enum: ["tavily", "firecrawl", "google"], defaultValue: "tavily" },
    ],
    capabilities: ["network:fetch"],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.query || typeof args.query !== "string") {
        return { toolName, success: false, output: "", error: "query must be a non-empty string", durationMs: Date.now() - start };
      }
...
const scrapeExtractSchemaTool: Tool = {
  definition: {
    ...
    params: [
      { name: "url", type: "string", description: "The URL to extract structured data from", required: true },
      { name: "schema", type: "string", description: "JSON schema string defining the structure to extract", required: true },
      { name: "multiple", type: "boolean", description: "Whether to extract multiple matching items", required: false, defaultValue: false },
    ],
    capabilities: ["network:fetch"],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      if (!args.schema || typeof args.schema !== "string") {
        return { toolName, success: false, output: "", error: "schema must be a non-empty JSON schema string", durationMs: Date.now() - start };
      }
      ...
      try {
        ...
        return { toolName, success: false, output: "", error: "schema must be valid JSON", durationMs: Date.now() - start };
      }
      ...
      for (const [key, prop] of Object.entries(properties)) {
        ...
        const metaMatch = html.match(
          new RegExp(`<meta[^>]*name\\s*=\\s*["']${key}["'][^>]*content\\s*=\\s*["']([^"']*)["'][^>]*>`, "i")
        );
        if (metaMatch) {
        ...
        const ogMatch = html.match(
          new RegExp(`<meta[^>]*property\\s*=\\s*["']og:${key}["'][^>]*content\\s*=\\s*["']([^"']*)["'][^>]*>`, "i")
        );
        if (ogMatch) {
        ...
        const itempropRegex = new RegExp(`<[^>]*itemprop\\s*=\\s*["']${key}["'][^>]*>([\\s\\S]*?)<\\/`, "i");
        ...
        if (extracted[key] === undefined) {
          const headingRegex = new RegExp(`<h[1-6][^>]*>[^<]*${key}[^<]*<\/h[1-6]>[\\s\\S]*?(?=<h[1-6]|$)`, "i");
          const headingMatch = headingRegex.exec(html);
          if (headingMatch) {
            extracted[key] = headingMatch[0].replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim().substring(0, 500);
          }
        }
      }
      ...
      return {
        ...
        success: false,
        output: "",
        error: `Schema extraction failed: ${error instanceof Error ? error.message : String(error)}`,
        durationMs: Date.now() - start,
      };
    }
  },
...
const scrapeMonitorTool: Tool = {
  definition: {
    ...
    params: [
      { name: "url", type: "string", description: "The URL to monitor for changes", required: true },
      { name: "interval_hours", type: "number", description: "How often to check for changes in hours (default: 24)", required: false, defaultValue: 24 },
      { name: "selector", type: "string", description: "CSS selector to monitor only a specific part of the page", required: false },
    ],
    capabilities: ["network:fetch"],
  },
    ...
    try {
      ...
      return {
        ...
        output: JSON.stringify({
          ...
          note: previous
            ? (changed ? "Content has changed since last check." : "No changes detected since last check.")
...
const scrapeExportTool: Tool = {
  definition: {
    name: "scrape_export",
    description: "Export scraped data (JSON array of items) to JSON, CSV, or Markdown table format.",
    params: [
      { name: "format", type: "string", description: "Export format for the data", required: false, enum: ["json", "csv", "markdown"], defaultValue: "json" },
      { name: "data", type: "string", description: "JSON array string of scraped items to export", required: true },
    ],
    capabilities: [],
  },
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      if (!args.data || typeof args.data !== "string") {
        return { toolName, success: false, output: "", error: "data must be a non-empty JSON array string", durationMs: Date.now() - start };
      }
      ...
      try {
        ...
        return { toolName, success: false, output: "", error: "data must be valid JSON", durationMs: Date.now() - start };
      }

      if (!Array.isArray(parsed)) {
        return { toolName, success: false, output: "", error: "data must be a JSON array", durationMs: Date.now() - start };
      }
